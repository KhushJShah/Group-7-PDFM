#%% Modified code with robust error handling
import pandas as pd
import numpy as np
...
def create_subplot(zipcode, train_data, train_pred, test_data, test_pred, county, state):
    ...
    if len(train_pred) > 0:
        ax1.plot(train_data.index[-len(train_pred):], train_pred, 
    ...
    plt.tight_layout()
    plot_path = f"ar_plots/{zipcode}_subplot.png"
...
def autoreg_grid_search(train_data, test_data, orders):
    ...
    for order in orders:
        try:
            ...
            heteroskedastic = False
            if len(residuals) > 5:  # Minimum data for ARCH test
                try:
            ...
            train_pred = model_fit.predict(start=order, end=len(train_data)-1)
            train_rmse = np.sqrt(mean_squared_error(train_data.iloc[order:], train_pred))
            train_mae = mean_absolute_error(train_data.iloc[order:], train_pred)
            ...
            if not np.isnan(test_rmse) and (best['order'] is None or test_rmse < best['test_rmse']):
                best.update({
                    'order': order,
...
def prepare_data(df):
    ...
    for zipcode in df['zipcode'].unique():
        ...
        # Grid search with specific orders
        orders = [4, 6, 10, 12, 14]
        best = autoreg_grid_search(train_data, test_data, orders)
        ...
        results.append({
            ...
            'heteroskedastic': best['heteroskedastic'],
...
def main():
    ...
    output_path = 'ar_results_fixed.csv'
    ...
    if not results_df.empty:
        ...
        print(f"Number of processed zipcodes: {len(results_df)}")
    else:
